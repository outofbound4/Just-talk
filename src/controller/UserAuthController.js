...
class UserAuthController {
    ...
    login(req, res) {
         ...
         */
            ...
            User.find({ 'email': email }, "_id mobile name email password statusbar profile_pic email_verified_at", function(errors, result) {
                if (errors) {
                    //sending errors to client page
                    return res.json({
                        status: 500,
                ...
                if (Object.keys(result).length !== 0) {
                    // it checks if email is verified or not
                    if (result[0].email_verified_at != null) {
                        ...
                        return res.json({
                            status: 423,
                            message: 'Verify email or Register again.',
                        });
                    }
                } else {
                    return res.json({
